...
runTestsOnModel() {
  ...
  for PROFILE in twitch_profile # cascade_profile_x0.25 twitch_profile_x0.25 FCCamazone_x0.25 Synthtic_x0.25
  ...
  do
    ...
    do
      ...
      docker run -d --rm -v `pwd`:/app -w /app --name alphartc_pyinfer --cap-add=NET_ADMIN -i -t 9e2342b020ff peerconnection_serverless receiver_pyinfer.json
      sleep 1
      ...
      python3 plot.py --title $modelName
